...
public static class TaskTargetParameterization // could be something like TargetParameters
{
    ...
    {
        List<Guid> playerIds = parameters.Where(x => x.Key.Contains("playerId"))
    ...
    {
        List<Guid> itemIds = parameters.Where(x => x.Key.Contains("item"))
            .Select(x => new Guid(x.Value)).ToList();
    ...
    {
        List<string> room = parameters.Where(x => x.Key.Contains("room"))
    ...
    {
        List<string> room = parameters.Where(x => x.Key.Contains("station"))
            .Select(x => x.Value).ToList();
